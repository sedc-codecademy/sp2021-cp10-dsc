...
import { QuizzesService } from "./QuizesService";
import chatBotBell from "../img-avatars/chatBotBell.png";
import chatButton from "../img-avatars/chatButton.svg";
import quizess from "../img-avatars/quizzes.svg";
...
export const UiService = {
  ...
  replyInfoMessage: function (infoName, infoMessage) {
    ...
    this.sleep().then(() => {
      ...
      this.chatHistory.scrollIntoView({ block: "end", behavior: "smooth" });
    });
  },
  ...
  printAcademyInfo: function (neededInfo, searchInput) {
    ...
    if (
      studyProgram[neededInfo.toLowerCase().replace(/\s/g, "")] !== undefined
    ) {
      this.replyInfoMessage(
        searchInput === undefined ? neededInfo : searchInput,
        studyProgram[neededInfo.toLowerCase().replace(/\s/g, "")]
      );
      this.sleep().then(() => {
  ...
  printConversationDone: function (choice) {
    if (choice === "Yes") {
      ...
      this.replyInfoMessage(choice, [
        "Ok amigo, I'll be here if you need me! \nJust ring the bell!",
      ]);
  ...
  printBellButton: function () {
    this.sleep().then(() => {
      this.helpersDiv.innerHTML += `<button id="waitingBellButton"><img src="${chatBotBell}" title="Ring me!!!" id="waitingBell" class="bell" height="50rem"></button>`;
      this.helpersDiv.scrollIntoView({ block: "end", behavior: "smooth" });
      document
        .getElementById("waitingBellButton")
  ...
  displayModalWindow: function (flag) {
    ...
    switch (flag) {
      case "quizzes":
        ...
        QuizzesService.form.scrollIntoView({
          block: "start",
          behavior: "smooth",
        });
        break;
...
export const testSomething = (smth) => {
  UiService.printAcademyInfo(smth);
};
