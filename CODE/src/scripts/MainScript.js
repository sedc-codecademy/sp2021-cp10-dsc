    ...
    setTimeout(() => {
      if (!isChatBotOpened) {
        if (firstTimePageLoaded) {
          maximizeSpeechBox.innerHTML =
            "Hey there partner! I'm the SEDC chatbot! Do you wanna talk?";
        } else {
          maximizeSpeechBox.innerHTML =
            "I'll be here if you need me just click on my handsome face!";
        }
...
if (ua.indexOf("firefox") > -1) {
  VoiceRecognitionService.voiceRecognitionBtn.style.display = "none";
  SearchInputService.inputButton.style.display = "block";
  VoiceRecognitionService.voiceRecognitionBtn.style.backgroundImage =
    "url(./src/img-avatars/send.svg)";
  VoiceRecognitionService.voiceRecognitionBtn.addEventListener(
    "click",
...
//Quizzes form submit
QuizzesService.form.addEventListener(
  "submit",
  QuizzesService.checkRightAnswers
);
