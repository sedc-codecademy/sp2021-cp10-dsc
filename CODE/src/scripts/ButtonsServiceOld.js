const ButtonsService2 = {
    ...
    mainButtonsLogic: function (branch, searchString) {
      ...
      UiService.replyMessages(
        searchString === undefined ? branch : searchString,
        DataService.cachedReplyMessages.ShortTalk.MainButtonReplies
      );

      switch (branch) {
        case "Testing":
          UiService.sleep().then(() => {
            ButtonsService.getTestingButtons(DataService.cachedData.Testing);
          });
          break;
        case "Academies":
          UiService.sleep().then(() => {
        ...
        case "Courses":
          UiService.sleep().then(() => {
            ButtonsService.getButtons(DataService.cachedData.Courses, "Courses");
          });
          break;
      }
    },
      ...
      for (const element of data) {
        if (element.nameId === buttonId) {
          if (element.studyPrograms !== undefined) {
            UiService.replyMessages(
              searchString === undefined ? element.name : searchString,
              element.reply
            );
            UiService.sleep().then(() => {
              ButtonsService.getButtons(element.studyPrograms, branchName);
            });
          } else {
            UiService.replyMessages(element.name, element.reply);
            UiService.sleep().then(() => {
              ButtonsService.getInfoButtons(element, branchName);
            });
          }
        }
      }
    ...
    recommendationsButtons: function () {
      ...
      for (let element in data) {
        ...
        if (element === "Testing") {
          ...
          const randomProgram = Math.floor(
            Math.random() * data[element][randomIndex].studyPrograms.length
          );
    ...
    findRecommendationButtonChoice: function (
      elementId,
      branchName,
      studyProgramName
    ) {
            ...
            for (let program of element.studyPrograms) {
              if (program.name === studyProgramName) {
                ...
                UiService.sleep().then(() => {
                  ...
                  branchName === "Academies"
                    ? AnimationsService.changeImageHead(studyProgramName)
                    : AnimationsService.changeImageHead("haralampiye");
                });
              }
            }
