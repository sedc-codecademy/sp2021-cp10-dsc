...
export const QuizzesService = {
  ...
  printQuizzesMenu: function () {
    AnimationsService.chatQuizzes.addEventListener("click", function () {
      ...
      if (!AnimationsService.isChatInitialized) {
        ...
        AnimationsService.chatQuizzes.addEventListener(
          "animationend",
          function () {
            UiService.changeQuizzesIconAndFunctionality(
              this.areQuizzesOpen,
              AnimationsService.chatQuizzes,
              QuizzesService.QuizzesWindow
            );
          }
        );
      } else {
      ...
      for (let quiz in DataService.cachedQuizzes.Quizzes) {
        quizzesInnerHTML += `<button id="${quiz}" class="btnCard" >
                        <div class="btnCardName"  id="btnCard.${quiz}" >${quiz.replace(
          /-/g,
          " "
        )}</div>
                        <div class="btnCardDescription" id="btnCardDescription.${quiz}">${
          DataService.cachedQuizzes.Descriptions[quiz]
        }</div>
                    </button> 
                    `;
      }
      ...
      QuizzesService.QuizzesWindow.addEventListener("click", (e) => {
        const buttonId = e.target.id;
        const WEB_DEVELOPMENT = "Web-Development";

        if (QuizzesService.isIdMatching(buttonId)) {
          QuizzesService.getQuiz(WEB_DEVELOPMENT);
        }
      });
  ...
  isIdMatching: (buttonId) => {
    const possibleIds = [
      "Web-Development",
      "btnCard.Web-Development",
      "btnCardDescription.Web-Development",
    ];

    return possibleIds.some((id) => id === buttonId);
  },
      ...
      quizzesInnerHTML += `
            ...
            <div class="quizQuestion">
                ...
                <div class="form-check">
                    <input type="radio" class="radio" name="q${
                      i + 1
                    }" value="A" id="first${i}">
                    <label for="first${i}" class="form-check-label">${
        questions[i].answer1
      }</label>
                </div>
                <div class="form-check">
                    ...
                    <label for="second${i}" class="form-check-label">${
        questions[i].answer2
      }</label>
                </div>
                <div class="form-check">
                    ...
                    <label for="third${i}" class="form-check-label">${
        questions[i].answer3
      }</label>
                </div>
                <div class="form-check">
                    ...
                    <label for="fourth${i}" class="form-check-label">${
        questions[i].answer4
      }</label>
                </div>
            </div>
  ...
  checkRightAnswers: function (event) {
    ...
    let userAnswers = [
      QuizzesService.form.q1.value,
      QuizzesService.form.q2.value,
      QuizzesService.form.q3.value,
      QuizzesService.form.q4.value,
      QuizzesService.form.q5.value,
      QuizzesService.form.q6.value,
      QuizzesService.form.q7.value,
      QuizzesService.form.q8.value,
      QuizzesService.form.q9.value,
      QuizzesService.form.q10.value,
      QuizzesService.form.q11.value,
      QuizzesService.form.q12.value,
      QuizzesService.form.q13.value,
      QuizzesService.form.q14.value,
      QuizzesService.form.q15.value,
    ];
    ...
    if (userAnswers.some(empty)) {
      ...
      QuizzesService.form.innerHTML =
        '<p class="somethingWentWrongMessage">All the questions must be answered before you submit them! <br><br>Please try again!</p>';
      return;
    }
    ...
    QuizzesService.form.style.display = "flex";
    QuizzesService.form.innerHTML = `<p class="somethingWentWrongMessage" id="quizResult">You got ${Math.ceil(
      score
    )}% of the questions right!</p>`;
  },
      ...
      Quizzes: {
        "Web-Development": [
          ...
          },
          {
            id: "21",
            question:
              "In CSS, select the appropriate option to style an element so that the next element would appear right next to it not underneath it:",
          ...
          },
          {
            id: "23",
            question:
              "Select the property used to create A space between the element’s border and inner content:",
            answer1: "border",
            answer2: "spacing",
            answer3: "padding",
            answer4: "margin",
